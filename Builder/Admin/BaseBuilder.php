...
class BaseBuilder extends GenericBaseBuilder
{
    ...
    {
        return $this->getGenerator()->getContainer();
    }
    ...
    protected function getDisplayAsColumns()
    {
        ...
        // TODO: test if the rest of the code is needed
    ...
    /**
     * Returns the template name available for the given parameters, whether the user bundle
     * defined or the admingenerated
     * 
     * @param  string $bundle Prefixed bundle name (e.g. AcmeDemoBundle)
     * @param  string $folder Folder where to find the template, usually the name of the builder (e.g. Entity)
     * @param  string $file   The file (or file path with filename)
     * @return string         Template name
     */
    public function getDefinedOrGeneratedTemplateName($bundle, $folder, $file)
    {
        $template = $this->getDefinedTemplateName($bundle, $folder, $file);

        if (!$this->getContainer()->get('templating')->exists($template)) {
            $template = $this->getGeneratedTemplateName($bundle, $folder, $file);
        }
    ...
    /**
     * Returns the template name the controller is expecting to find defined by the user bundle
     * 
     * @return [type] [description]
     */
    public function getDefinedTemplateName($bundle, $folder, $file)
    {
        return sprintf('%s:%s:%s', $bundle, $folder, $file);
    }

    /**
     * Returns the template name generated by this type of action (YamlKey)
     * 
     * @return [type] [description]
     */
    public function getGeneratedTemplateName($bundle, $folder, $file)
    {
        return sprintf('Admingenerated/%s/Resources/views/%s/%s', $bundle, $folder, $file);
    }
}
