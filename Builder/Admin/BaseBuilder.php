...
class BaseBuilder extends GenericBaseBuilder
{
    ...
    {
        $template = sprintf('%s:%s:%s', $bundle, $folder, $file);

        if (!$this->getContainer()->get('templating')->exists($template)) {
            $template = sprintf('Admingenerated/%s/Resources/views/%s/%s', $bundle, $folder, $file);
        }
    ...
    /**
     * Returns the user defined form class if exists, the admingenerated one otherwise
     * 
